...
class AutoClicker:
    def __init__(self, root):
        ...
        # Przywrócony przycisk Tylda (toggle klawisz)
    ...
    # Funkcja toggle – otwiera lub zamyka okno testowego zrzutu (F9)
    ...
    def rebind_hotkeys(self):
        ...
        toggle_key = self.toggle_key_entry.get()  # Tilde – klawisz włączania/wyłączania
        test_key = self.test_key_entry.get()      # Ustal wzorzec – F9
        new_test_key = self.new_test_key_entry.get()  # Nowy screenshot – F10
        keyboard.add_hotkey(toggle_key, self.toggle_clicking)
        keyboard.add_hotkey(test_key, self.toggle_test_window)
        keyboard.add_hotkey(new_test_key, self.new_screenshot)
        self.log_message(f"Hotkeys ustawione: Włącz/wyłącz = {toggle_key}, Ustal wzorzec = {test_key}, Nowy screenshot = {new_test_key}")
