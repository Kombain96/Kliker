...
class AutoClicker:
    def __init__(self, root):
        ...
        self.templates = []  # Lista przechowująca wszystkie ustalone wzorce
        self.clicked_on_template = False  # Flaga, by nie wykonywać wielokrotnych kliknięć przy jednym wykryciu
        ...
        # Zmieniona etykieta – zamiast "Klawisz testowego zrzutu" mamy "Ustal wzorzec"
        ttk.Label(frame_keys, text="Ustal wzorzec (F9):").pack(side=tk.LEFT)
        ...
        # Usunięto Panic – więc nie tworzymy pola dla niego.
        # Nowy Screenshot – tylko aktualizacja zrzutu w oknie testowym
        ttk.Label(frame_keys, text="Nowy Screenshot (F10):").pack(side=tk.LEFT)
        self.new_test_key_entry = ttk.Entry(frame_keys, width=5)
        self.new_test_key_entry.pack(side=tk.LEFT, padx=2)
        self.new_test_key_entry.insert(0, "F10")
        ...
        self.template_button = ttk.Button(frame_extra, text="Pokaż wzorce", command=self.show_template_window)
        self.template_button.pack(side=tk.LEFT, padx=5)

        # -------------------- Przypisanie hotkeyów --------------------
        self.rebind_hotkeys()

        # -------------------- Uruchomienie wątku śledzenia --------------------
    ...
    def create_preview_window(self):
        ...
        # Placeholder z podzielonym tekstem, czerwony kolor
        self.real_time_preview_label = tk.Label(
            self.preview_window,
            text="Zrób testowy zrzut ekranu\noraz uruchom program aby wyświetlić podgląd",
    ...
    def show_template_window(self):
        ...
        template_window.title("Wzorce")
        # Przycisk do usuwania wzorców
        btn_clear = ttk.Button(template_window, text="Usuń wzorce", command=self.remove_templates)
        btn_clear.pack(side=tk.TOP, pady=5)
        if not self.templates:
            lbl = tk.Label(template_window, text="Brak wzorców do wyświetlenia.")
            lbl.pack()
        else:
            # Wyświetlamy wszystkie wzorce obok siebie
            for tmpl in self.templates:
                im_pil = Image.fromarray(tmpl)
                photo = ImageTk.PhotoImage(im_pil)
                lbl = tk.Label(template_window, image=photo)
                lbl.image = photo  # zachowujemy referencję
                lbl.pack(side=tk.LEFT, padx=5, pady=5)

    def remove_templates(self):
        self.templates = []
        self.log_message("Wszystkie wzorce zostały usunięte.")

    # -------------------- Blokada i odblokowanie interfejsu --------------------
    def lock_interface(self):
        widgets = (self.key_entries + self.interval_entries +
                   [self.mouse_interval_entry, self.toggle_key_entry,
                    self.test_key_entry, self.new_test_key_entry, self.rebind_button, self.select_region_button,
    ...
    # -------------------- Testowy zrzut ekranu i ustalanie wzorców --------------------
    # Funkcja toggle – otwiera/zamyka okno testowego zrzutu (F9)
    ...
    # Funkcja pobierająca nowy screenshot (F10) – aktualizuje obraz w oknie testowym
    def new_screenshot(self):
        if hasattr(self, "test_window") and self.test_window.winfo_exists():
            self.test_screenshot()
        else:
            self.log_message("Okno testowego zrzutu nie jest otwarte. Naciśnij F9, aby je otworzyć.")

    def test_screenshot(self):
        # Pobierz zrzut ekranu z wybranego regionu lub całego ekranu
    ...
    def test_mouse_up(self, event):
        ...
        self.log_message(f"Testowy obszar zaznaczony: {(x1, y1, x2-x1, y2-y1)}")
        # Dodajemy zaznaczony obszar jako nowy wzorzec
        region = self.test_original_img[y1:y2, x1:x2]
        if region.size > 0:
            self.templates.append(region)
            self.log_message("Dodano wzorzec. Łączna liczba wzorców: " + str(len(self.templates)))
    ...
    def rebind_hotkeys(self):
        ...
        toggle_key = self.test_key_entry.get()  # F9 – ustal wzorzec (toggle okna testowego)
        new_test_key = self.new_test_key_entry.get()  # F10 – nowy screenshot
        # Przypisujemy odpowiednie funkcje
        keyboard.add_hotkey(toggle_key, self.toggle_test_window)
        keyboard.add_hotkey(new_test_key, self.new_screenshot)
        self.log_message(f"Hotkeys ustawione: Ustal wzorzec = {toggle_key}, Nowy Screenshot = {new_test_key}")
    ...
    def save_config(self):
        config = {
            ...
            "toggle_key": self.test_key_entry.get(),
            "new_test_key": self.new_test_key_entry.get(),
    ...
    def load_config(self):
        try:
            ...
            self.test_key_entry.insert(0, config.get("toggle_key", "F9"))
            self.new_test_key_entry.delete(0, tk.END)
            self.new_test_key_entry.insert(0, config.get("new_test_key", "F10"))
    ...
    # -------------------- Zaawansowane wyszukiwanie wzorca --------------------
        ...
        while True:
            if self.tracking_enabled:
                try:
                    ...
                    if self.templates:
                        for tmpl in self.templates:
                            dst = self.advanced_image_search(img_rgb, tmpl)
                            if dst is not None:
                                pts = np.int32(dst)
                        ...
                        self.log_message("Brak wzorców. Użyj 'Ustal wzorzec' (F9) i wybierz obszary wzorców.")
