...
class AutoClicker:
    def __init__(self, root):
        ...
        # Początkowo klikacz i śledzenie są wyłączone
        self.running = False
        self.tracking_enabled = False
        
        # Konfiguracja pól dla klawiszy oraz interwałów
        self.key_entries = []
        ...
        # Konfiguracja regionu wyszukiwania obiektu
        ...
        # Hotkey: Tylda (`) przełącza klikacz i śledzenie, ESC – panic button.
        keyboard.add_hotkey('`', self.toggle_clicking)
        keyboard.add_hotkey('esc', self.panic_stop)
        
        # Uruchamiamy wątek śledzenia, który będzie działał cały czas, ale reagować będzie tylko, gdy tracking_enabled=True
    ...
    def toggle_clicking(self):
        # Przełączamy jednocześnie działanie klikacza i śledzenie obiektów
        if self.running or self.tracking_enabled:
            self.stop_clicking()
            self.tracking_enabled = False
            self.status_label.config(text="Stan: OFF", foreground="red")
            self.log_message("Klikacz i śledzenie obiektów zatrzymane.")
        else:
            self.start_clicking()
            self.tracking_enabled = True
            self.status_label.config(text="Stan: ON", foreground="green")
            self.log_message("Klikacz i śledzenie obiektów uruchomione.")
