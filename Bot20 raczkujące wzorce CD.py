...
class AutoClicker:
    def __init__(self, root):
        ...
        # Dodajemy nowe pole: klawisz do pobierania kolejnego testowego zrzutu
        ttk.Label(frame_keys, text="Klawisz nowego zrzutu testowego:").pack(side=tk.LEFT)
    ...
    def create_preview_window(self):
        ...
        # Placeholder – kwadrat z informacją
        self.real_time_preview_label = tk.Label(
            self.preview_window,
            text="Zrób testowy zrzut ekranu oraz uruchom program aby wyświetlić podgląd",
    ...
    def show_template_window(self):
        if not self.templates:
            self.log_message("Nie ustalono wzorców. Wykonaj testowy zrzut i wybierz obszary wzorców.")
            return
    ...
    def test_screenshot(self):
        ...
        # Jeśli okno testowe już istnieje, zaktualizuj jego obraz; w przeciwnym razie utwórz nowe okno
    ...
    def test_mouse_up(self, event):
        ...
        # Ustalony obszar dodajemy jako nowy wzorzec
    ...
    def rebind_hotkeys(self):
        ...
        keyboard.add_hotkey(new_test_key, self.test_screenshot)
        self.log_message(f"Hotkeys ustawione: Toggle = {toggle_key}, Panic = {panic_key}, Test = {test_key}, Nowy test = {new_test_key}")
    ...
    def click_loop(self, keys_intervals):
        while self.running:
            for key, interval in keys_intervals:
                if not self.running:
                    break
                # Dla dynamicznych klawiszy – prawy przycisk obsługiwany jest w track_object
    ...
    def advanced_image_search(self, screenshot, template):
        ...
        good_matches = []
        for m, n in matches:
            # Zwiększony próg tolerancji
            if m.distance < 0.85 * n.distance:
                good_matches.append(m)
        # Minimalna liczba dobrych dopasowań obniżona do 8
        ...
        while True:
            if self.tracking_enabled:
                try:
                    ...
                    if self.templates:
                        # Dla każdego wzorca próbujemy znaleźć dopasowanie
                        for tmpl in self.templates:
                            ...
                            if dst is not None:
                                ...
                                # Jeśli nie wykonano jeszcze kliknięcia dla tego wykrycia
                        ...
                        self.log_message("Brak wzorców. Użyj 'Testowy zrzut' i wybierz obszary wzorców.")
