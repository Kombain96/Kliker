...
class AutoClicker:
    def __init__(self, root):
        ...
        self.style.configure("TFrame", background="#E8F6F3")
        self.style.configure("TLabel", background="#E8F6F3", foreground="#333333", font=("Arial", 10))
        self.style.configure("TButton", background="#A7C7E7", foreground="#000000", font=("Arial", 10))
        ...
        # Umieszczone w lewym panelu (kolumna 0)
        self.key_entries = []
        ...
        self.log_text.grid(row=17, column=0, columnspan=2, padx=5, pady=5, sticky='we')

        # -------------------- Sekcja stałych klawiszy F1-F12 (kompaktowo, obok dynamicznych) --------------------
        self.fixed_frame = ttk.Frame(root)
        self.fixed_frame.grid(row=0, column=1, rowspan=17, padx=5, pady=5, sticky='n')
        ...
        # Rozmieść F1-F12 w dwóch kolumnach (po 6 wierszy)
        for i in range(1, 13):
            row = ((i - 1) % 6) + 1
            col = 0 if i <= 6 else 2
        ...
        # -------------------- Utworzenie okna podglądu w czasie rzeczywistym --------------------
    ...
    def lock_interface(self):
        widgets = (self.key_entries + self.interval_entries +
                   [self.mouse_interval_entry, self.toggle_key_entry, self.panic_key_entry,
                    ...
                    self.red_min_entry, self.green_max_entry, self.blue_max_entry,
    ...
    # -------------------- Testowy zrzut ekranu i wizualna edycja tolerancji koloru --------------------
    def test_screenshot(self):
        # Jeśli okno testowe jest już otwarte, zamknij je (toggle)
        if hasattr(self, "test_window") and self.test_window.winfo_exists():
            self.test_window.destroy()
            return
        # Wykonaj zrzut ekranu wybranego regionu lub całego ekranu
        if self.selected_region:
            x, y, w, h = self.selected_region
            screenshot = pyautogui.screenshot(region=(x, y, w, h))
        else:
            screenshot = pyautogui.screenshot()
        # Zachowaj oryginalny obraz jako tablicę NumPy
        self.test_original_img = np.array(screenshot)
        # Utwórz nowe okno dla testowego zrzutu
        ...
        # Dodaj panel do ustawiania tolerancji koloru w oknie testowym
        tolerance_frame = ttk.Frame(self.test_window)
        tolerance_frame.pack(fill="x", padx=5, pady=5)
        ttk.Label(tolerance_frame, text="Tolerancja koloru:").pack(anchor="w")
        self.test_red_scale = tk.Scale(tolerance_frame, from_=0, to=255, orient="horizontal", label="Min Red", command=lambda x: self.update_test_image())
        self.test_red_scale.set(int(self.red_min_entry.get()))
        self.test_red_scale.pack(fill="x", padx=5)
        self.test_green_scale = tk.Scale(tolerance_frame, from_=0, to=255, orient="horizontal", label="Max Green", command=lambda x: self.update_test_image())
        self.test_green_scale.set(int(self.green_max_entry.get()))
        self.test_green_scale.pack(fill="x", padx=5)
        self.test_blue_scale = tk.Scale(tolerance_frame, from_=0, to=255, orient="horizontal", label="Max Blue", command=lambda x: self.update_test_image())
        self.test_blue_scale.set(int(self.blue_max_entry.get()))
        self.test_blue_scale.pack(fill="x", padx=5)

    def update_test_image(self):
        if not hasattr(self, "test_original_img"):
            return
        img_rgb = self.test_original_img.copy()
        red_min = int(self.test_red_scale.get())
        green_max = int(self.test_green_scale.get())
        blue_max = int(self.test_blue_scale.get())
        ...
        if valid_contour is not None:
            ...
            cv2.drawContours(annotated_img, [valid_contour], -1, (0, 0, 255), 2)
            mask_roi = np.zeros_like(mask)
            cv2.drawContours(mask_roi, [valid_contour], -1, 255, -1)
            mean_val = cv2.mean(img_rgb, mask=mask_roi)
            mean_color = (int(mean_val[2]), int(mean_val[1]), int(mean_val[0]))
            text = f"Kształt: {shape_filter} Kolor: {mean_color}"
            cv2.putText(annotated_img, text, (x_rect, y_rect - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
    ...
    def start_clicking(self):
        keys_intervals = []
        # Dynamiczne klawisze wpisane przez użytkownika
