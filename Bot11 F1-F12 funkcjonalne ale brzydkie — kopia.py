...
class AutoClicker:
    def __init__(self, root):
        ...
        # Sekcja stałych klawiszy F1 - F12 z możliwością ustawienia czasu
        frame_fixed = ttk.Frame(root)
        frame_fixed.grid(row=17, column=0, padx=5, pady=2, sticky='w')
        ttk.Label(frame_fixed, text="Stałe klawisze F1 - F12:").pack(anchor='w')
        self.fixed_key_entries = {}
        # Każdy klawisz F1-F12 umieszczony w osobnej ramce
        for i in range(1, 13):
            f_frame = ttk.Frame(root)
            f_frame.grid(row=17+i, column=0, padx=5, pady=2, sticky='w')
            label = ttk.Label(f_frame, text=f"F{i}:")
            label.pack(side=tk.LEFT)
            time_entry = ttk.Entry(f_frame, width=5)
            time_entry.pack(side=tk.LEFT, padx=5)
            self.fixed_key_entries[f"F{i}"] = time_entry

        # Panel logów
        self.log_text = ScrolledText(root, height=10, width=50)
        # Umieszczamy logi poniżej stałych klawiszy
        self.log_text.grid(row=30, column=0, columnspan=2, padx=5, pady=5)
    ...
    def create_preview_window(self):
        ...
        self.real_time_preview_label = tk.Label(self.preview_window)
    ...
    def lock_interface(self):
        ...
        # Blokujemy również stałe klawisze
    ...
    # ----------------------- Testowy zrzut ekranu i ręczne zaznaczanie obiektu -----------------------
    ...
    def test_mouse_move(self, event):
        try:
            if self.test_canvas.winfo_exists():
                self.test_canvas.delete("sel")
                self.test_canvas.create_rectangle(self.test_start_x, self.test_start_y, event.x, event.y, outline="red", width=2, tag="sel")
        except tk.TclError:
            pass
    ...
    def test_mouse_up(self, event):
        ...
        self.log_message(f"Testowy obiekt zaznaczony: {self.test_selected_region}")
            ...
            if self.tracking_enabled:
                try:
                    ...
                    if valid_contour is not None:
                        ...
                        # Oblicz średni kolor wewnątrz konturu
                    ...
                    # Aktualizacja podglądu: przekształcenie obrazu do formatu kompatybilnego z Tkinter
