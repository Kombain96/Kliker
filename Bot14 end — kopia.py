...
class AutoClicker:
    def __init__(self, root):
        ...
        # -------------------- Ustawienia tolerancji koloru (główne GUI) --------------------
    ...
    # -------------------- Testowy zrzut ekranu i automatyczne ustawianie tolerancji koloru --------------------
    def test_screenshot(self):
        ...
        # Zachowaj oryginalny obraz jako tablicę NumPy (w formacie RGB)
        ...
        # Powiąż zdarzenia myszy do zaznaczania obszaru
    ...
    def test_mouse_up(self, event):
        ...
        # Automatyczne obliczenie tolerancji koloru na podstawie wybranego obszaru
        region = self.test_original_img[y1:y2, x1:x2]
        if region.size > 0:
            avg_color = np.mean(region, axis=(0,1))  # [R, G, B]
            margin = 10
            red_min = max(0, int(avg_color[0]) - margin)
            green_max = min(255, int(avg_color[1]) + margin)
            blue_max = min(255, int(avg_color[2]) + margin)
            self.red_min_entry.delete(0, tk.END)
            self.red_min_entry.insert(0, str(red_min))
            self.green_max_entry.delete(0, tk.END)
            self.green_max_entry.insert(0, str(green_max))
            self.blue_max_entry.delete(0, tk.END)
            self.blue_max_entry.insert(0, str(blue_max))
            self.log_message(f"Automatycznie ustawiono tolerancję koloru: Red min = {red_min}, Green max = {green_max}, Blue max = {blue_max}")
