...
class AutoClicker:
    def __init__(self, root):
        ...
        self.templates = []
        self.clicked_on_template = False  # flaga zapobiegająca wielokrotnemu klikaniu przy jednym wykryciu

        # Domyślne ustawienia zaawansowane dla ORB, FLANN, RANSAC oraz wieloskalowości
        self.advanced_params = {
            "normalization": True,          # Czy stosować equalizeHist, co poprawia kontrast
            "contrast": 1.0,                # Współczynnik kontrastu (alpha, >1 zwiększa kontrast)
            "orb_nfeatures": 1500,          # Liczba cech wykrywanych przez ORB
            "orb_scaleFactor": 1.2,         # Skala między poziomami piramidy (np. 1.2)
            "orb_nlevels": 8,               # Liczba poziomów piramidy
            "flann_table_number": 6,        # FLANN – liczba tabel LSH
            "flann_key_size": 12,           # FLANN – długość klucza
            "flann_multi_probe_level": 1,   # FLANN – poziom dodatkowych wyszukiwań
            "flann_checks": 50,             # FLANN – liczba sprawdzanych drzew
            "ransac_reproj_threshold": 5.0, # Próg reprojekcji w RANSAC
    ...
    def open_advanced_window(self):
        ...
        self.adv_window.title("Zaawansowane - ORB, FLANN, RANSAC i Multi-scale")
        row = 0
        # Normalizacja
        ...
        row += 1
        # Parametry ORB
        tk.Label(self.adv_window, text="ORB - liczba cech (nfeatures):").grid(row=row, column=0, sticky="w")
        self.orb_nfeatures_entry = ttk.Entry(self.adv_window, width=10)
        self.orb_nfeatures_entry.insert(0, str(self.advanced_params["orb_nfeatures"]))
        tk.Label(self.adv_window, text="Maksymalna liczba cech wykrywanych przez ORB").grid(row=row, column=2, sticky="w")
        self.orb_nfeatures_entry.grid(row=row, column=1)
        row += 1
        tk.Label(self.adv_window, text="ORB - scaleFactor:").grid(row=row, column=0, sticky="w")
        self.orb_scaleFactor_entry = ttk.Entry(self.adv_window, width=10)
        self.orb_scaleFactor_entry.insert(0, str(self.advanced_params["orb_scaleFactor"]))
        tk.Label(self.adv_window, text="Skala między poziomami piramidy (np. 1.2)").grid(row=row, column=2, sticky="w")
        self.orb_scaleFactor_entry.grid(row=row, column=1)
        row += 1
        tk.Label(self.adv_window, text="ORB - nlevels:").grid(row=row, column=0, sticky="w")
        self.orb_nlevels_entry = ttk.Entry(self.adv_window, width=10)
        self.orb_nlevels_entry.insert(0, str(self.advanced_params["orb_nlevels"]))
        tk.Label(self.adv_window, text="Liczba poziomów piramidy").grid(row=row, column=2, sticky="w")
        self.orb_nlevels_entry.grid(row=row, column=1)
        row += 1
        # Parametry FLANN
        tk.Label(self.adv_window, text="FLANN - table_number:").grid(row=row, column=0, sticky="w")
        self.flann_table_entry = ttk.Entry(self.adv_window, width=10)
        self.flann_table_entry.insert(0, str(self.advanced_params["flann_table_number"]))
        tk.Label(self.adv_window, text="Liczba tabel LSH").grid(row=row, column=2, sticky="w")
        self.flann_table_entry.grid(row=row, column=1)
        row += 1
        tk.Label(self.adv_window, text="FLANN - key_size:").grid(row=row, column=0, sticky="w")
        self.flann_key_entry = ttk.Entry(self.adv_window, width=10)
        self.flann_key_entry.insert(0, str(self.advanced_params["flann_key_size"]))
        tk.Label(self.adv_window, text="Długość klucza").grid(row=row, column=2, sticky="w")
        self.flann_key_entry.grid(row=row, column=1)
        row += 1
        tk.Label(self.adv_window, text="FLANN - multi_probe_level:").grid(row=row, column=0, sticky="w")
        self.flann_multi_probe_entry = ttk.Entry(self.adv_window, width=10)
        self.flann_multi_probe_entry.insert(0, str(self.advanced_params["flann_multi_probe_level"]))
        tk.Label(self.adv_window, text="Poziom dodatkowych wyszukiwań").grid(row=row, column=2, sticky="w")
        self.flann_multi_probe_entry.grid(row=row, column=1)
        row += 1
        ...
        tk.Label(self.adv_window, text="Liczba sprawdzanych drzew").grid(row=row, column=2, sticky="w")
        ...
        tk.Label(self.adv_window, text="Próg reprojekcji - mniejsza wartość = bardziej rygorystyczne dopasowanie").grid(row=row, column=2, sticky="w")
    ...
    def save_advanced_params(self):
        try:
            ...
            self.advanced_params["orb_nfeatures"] = int(self.orb_nfeatures_entry.get())
            self.advanced_params["orb_scaleFactor"] = float(self.orb_scaleFactor_entry.get())
            self.advanced_params["orb_nlevels"] = int(self.orb_nlevels_entry.get())
            self.advanced_params["flann_table_number"] = int(self.flann_table_entry.get())
            self.advanced_params["flann_key_size"] = int(self.flann_key_entry.get())
            self.advanced_params["flann_multi_probe_level"] = int(self.flann_multi_probe_entry.get())
    ...
    # ---------- Zaawansowane wyszukiwanie obrazu z ORB, FLANN, RANSAC i wieloskalowością ----------
    def advanced_image_search(self, screenshot, template):
        ...
        for scale in scales:
            try:
                ...
                continue
            # Detektor ORB z ustawieniami z GUI
            orb = cv2.ORB_create(nfeatures=int(self.advanced_params["orb_nfeatures"]),
                                 scaleFactor=float(self.advanced_params["orb_scaleFactor"]),
                                 nlevels=int(self.advanced_params["orb_nlevels"]))
            kp1, des1 = orb.detectAndCompute(resized_template, None)
            kp2, des2 = orb.detectAndCompute(img_gray, None)
            if des1 is None or des2 is None:
                continue
            # Ustawienia FLANN dla ORB (LSH)
            FLANN_INDEX_LSH = 6
            index_params = dict(algorithm=FLANN_INDEX_LSH,
                                table_number=int(self.advanced_params["flann_table_number"]),
                                key_size=int(self.advanced_params["flann_key_size"]),
                                multi_probe_level=int(self.advanced_params["flann_multi_probe_level"]))
