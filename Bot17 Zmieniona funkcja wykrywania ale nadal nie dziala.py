...
class AutoClicker:
    def __init__(self, root):
        ...
        self.clicked_on_template = False  # Flaga, by nie klikać wielokrotnie na tym samym wykryciu
    ...
    def show_template_window(self):
        if self.template_img is None:
            self.log_message("Nie ustalono wzorca. Użyj 'Testowy zrzut' i zaznacz obszar wzorca.")
            return
        template_window = tk.Toplevel(self.root)
        template_window.title("Wzorzec")
        im_pil = Image.fromarray(self.template_img)
        photo = ImageTk.PhotoImage(im_pil)
        label = tk.Label(template_window, image=photo)
    ...
    def save_config(self):
        config = {
            ...
            # Nie dodajemy już "RightClick" do listy dynamicznych klawiszy
    ...
    # -------------------- Auto klikacz (dla pozostałych dynamicznych klawiszy) --------------------
    def click_loop(self, keys_intervals):
        while self.running:
            for key, interval in keys_intervals:
                if not self.running:
                    break
                # Dla dynamicznych klawiszy (pomijamy RightClick, który obsługujemy w track_object)
    ...
    # -------------------- Śledzenie obiektu z wykorzystaniem detekcji wzorca --------------------
                ...
                try:
                    ...
                    if self.template_img is not None:
                        ...
                        if dst is not None:
                            ...
                            # Jeśli jeszcze nie wykonano kliknięcia dla bieżącego wykrycia wzorca
